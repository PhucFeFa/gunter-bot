...
try { db.prepare('ALTER TABLE users ADD COLUMN limitDisabled INTEGER DEFAULT 0').run(); } catch (e) { /* Cột đã tồn tại */ }
...
function transferMoney(fromUserId, toUserId, amount) {
    ...
    if (!isAdmin) {
        ...
        const isLimitDisabled = sender.limitDisabled === 1;
...
module.exports = {
    ...
    setTransferLimit: async (userId, disabled) => {
        db.prepare('UPDATE users SET limitDisabled = ? WHERE userId = ?').run(disabled ? 1 : 0, userId);
    },
    getTransferLimit: async (userId) => {
        const row = db.prepare('SELECT limitDisabled FROM users WHERE userId = ?').get(userId);
        return row && row.limitDisabled === 1;
    }
};
